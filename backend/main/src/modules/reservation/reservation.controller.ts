...
export class ReservationController {
  ...
  */
  ...
  @UseGuards(AuthGuard('jwt'))
  @Get('findAllBTableAndClient/:id')
  findAllByTableAndClient(@Param('id') id: number, @Request() req): Promise<Reservation[]> {
    if (['admin', 'owner', 'user_plus', 'user'].includes(req.user.permiss))
      return this.ReservationService.findAllByTableAndClient(id, req.user.client_id);
  }

  @UseGuards(AuthGuard('jwt'))
  @Get('findAllByTableAndClient/:user_id')
  findAllByUserAndClient(@Param('user_id') user_id: number, @Request() req): Promise<Reservation[]> {
    if (['admin', 'owner', 'user_plus', 'user'].includes(req.user.permiss))
      return this.ReservationService.findAllByUserAndClient(user_id, req.user.client_id);
  }

  @UseGuards(AuthGuard('jwt'))
  @Get('findAllByDateAndClient/:date')
  findAllByDateAndClient(@Param() date : Date , @Request() req): Promise<Reservation[]> {
  ...
  */
  ...
  @UseGuards(AuthGuard('jwt'))
  @Get('findAllbyDate/:date')
  findAllbyDate(@Body('date') date: Date): Promise<Reservation[]> {
  ...
  @UseGuards(AuthGuard('jwt'))
  @Get('findbyId/:id')
  findAllbyId(@Param('id') id: number, @Request() req): Promise<Reservation> {
